...
const NamazVakitleri: React.FC<{ onGoHome: () => void }> = ({ onGoHome }) => {
    ...
    const fetchPrayerTimes = async (city: string, country: string) => {
        ...
        setError(null);
        try {
            // Use Aladhan API directly which supports city and country
            const response = await axios.get(`https://api.aladhan.com/v1/timingsByCity`, {
                params: { city, country, method: 13 } // 13 is Diyanet İşleri Başkanlığı
            });
    ...
    const handleGeolocate = () => {
        ...
        navigator.geolocation.getCurrentPosition(
            async (position) => {
                ...
                setError(null);
                try {
                    // Get city/country for display with BigDataCloud (No CORS)
                    const geoResponse = await axios.get(`https://api.bigdatacloud.net/data/reverse-geocode-client?latitude=${latitude}&longitude=${longitude}&localityLanguage=tr`);
