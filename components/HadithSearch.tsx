...
const HadithSearch: React.FC<{ onGoHome: () => void }> = ({ onGoHome }) => {
    ...
    const handleShareHistoryItem = async (e: React.MouseEvent, item: HistoryItem) => {
        ...
        try {
            ...
            const shareUrl = `${window.location.origin}${window.location.pathname}#/?module=hadith&v=2&data=${encodeURIComponent(encodedData)}`;
                                        ...
                                        {msg.content.hadiths?.map((hadith, hIndex) => {
                                            ...
                                            return (
                                                <div
                                                    ...
                                                    className="w-full text-left p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg space-y-3 border-l-4 border-teal-500 hover:border-teal-600 dark:hover:border-teal-500 hover:shadow-lg transform hover:-translate-y-1 transition-all duration-300 cursor-pointer"
                                                >
