...
const FiqhChat: React.FC<{ onGoHome: () => void }> = ({ onGoHome }) => {
    ...
    useEffect(() => {
        try {
            ...
            const importedDataString = sessionStorage.getItem('importedDataFor_fiqh');
            if (importedDataString) {
                sessionStorage.removeItem('importedDataFor_fiqh');
...
Cevabın her zaman bu yapılandırılmış formatta ve JSON olarak dönmelidir. Eğer bir bölüm için (hadis, ayet, mezhep görüşü) bilgi bulamazsan, o bölüm için boş bir dizi [] döndür, ama anahtarı (key) asla yanıttan çıkarma.`,
    ...
    const handleShareHistoryItem = async (e: React.MouseEvent, item: HistoryItem) => {
        ...
        try {
            ...
            const shareUrl = `${window.location.origin}${window.location.pathname}#/?module=fiqh&v=2&data=${encodeURIComponent(encodedData)}`;
                                                    ...
                                                    <div key={i} className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border-l-4 border-amber-500 flex flex-col justify-between">
                                                        <div>
                                                            ...
                                                            <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                                                                <HighlightableText>{item.position}</HighlightableText>
                                                            </p>
                                                        </div>
                                                        {item.source && (
                                            ...
                                            <div className="space-y-4">
                                                {msg.content.relevantQuranVerses.map((verse, i) => (
                                                    <div key={i} className="p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                                                        ...
                                                        <p className="text-gray-600 dark:text-gray-400 italic mb-3">
                                                            "<HighlightableText>{verse.turkishText}</HighlightableText>"
                                                        </p>
